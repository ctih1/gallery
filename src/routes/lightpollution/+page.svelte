...
<div class="flex flex-col mt-8">
    <Input id="pollution" type="range" min=16 max=22 step=0.1 bind:value={pollutionValue}>
        Sky brightness: <input bind:value={pollutionValue} class="w-8 bg-zinc-900"> mag/arcsec²
    </Input>
</div>
...
<div class="h-96">
    <img style={`filter: blur(${relative}px) contrast(${1-relative}) saturate(${1-(relative*0.5)})`} class="w-96 aspect-square absolute object-cover" alt="sky at night" src="https://images.pexels.com/photos/13041555/pexels-photo-13041555.jpeg">
    <div style={`opacity: ${relative > 0.2 ? (relative - 0.2) * 0.9 : 0}`} class="bg-radial from-[#ffb066] to-[#9bb0c4] w-96 aspect-square absolute mix-blend-screen"></div>
</div>
